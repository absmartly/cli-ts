...
if (authMethod === 'oauth-jwt') {
  ...
  if (!token) {
    console.error(`No OAuth token found for profile "${profileName}". Run: abs auth login --profile ${profileName}`);
    process.exit(1);
  }
  ...
  if (!apiKey) {
    console.error(`No API key found for profile "${profileName}". Run: abs auth login --profile ${profileName}`);
    process.exit(1);
  }
