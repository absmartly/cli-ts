...
async function resolveUserValues(client: import('../../api-client/api-client.js').APIClient, raw: string): Promise<UserId[]> {
  const refs = parseCSV(raw);
  const allNumeric = refs.every(r => !isNaN(parseInt(r, 10)) && String(parseInt(r, 10)) === r.trim());
...
async function resolveRoleValues(client: import('../../api-client/api-client.js').APIClient, raw: string): Promise<RoleId[]> {
  const refs = parseCSV(raw);
  const allNumeric = refs.every(r => !isNaN(parseInt(r, 10)) && String(parseInt(r, 10)) === r.trim());
