...
export const estimateParticipantsCommand = new Command('estimate-participants')
  ...
  .option(
    ...
    'application name or ID (repeatable)',
    (val: string, prev: string[]) => [...prev, val],
    [] as string[]
  )
  .option(
    '--from <date>',
    'start of observation window (default: 30d ago; accepts relative dates, ISO 8601, or ms epoch)',
    '30d'
  )
    ...
    withErrorHandling(async (options) => {
      ...
      for (let i = 0; i < result.data.rows.length; i++) {
        ...
        if (row.unitCount !== undefined) {
          console.log(
            chalk.green(`  Estimated max participants: ${row.unitCount.toLocaleString()}`)
          );
        }
        if (row.firstExposure !== undefined)
          console.log(`  First exposure: ${formatTimestampMs(row.firstExposure)}`);
        if (row.lastExposure !== undefined)
          console.log(`  Last exposure:  ${formatTimestampMs(row.lastExposure)}`);
      }
