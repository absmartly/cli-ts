  ...
  .action(
    withErrorHandling(async (nameOrId: string, options) => {
      ...
      if (globalOptions.output === 'rendered') {
        ...
        const customFields = exp.custom_section_field_values as
        ...
        lines.push(`---`);
        lines.push(
          `*Created: ${summary.created_at || 'N/A'} | Updated: ${summary.updated_at || 'N/A'} | Started: ${summary.start_at || 'N/A'} | Stopped: ${summary.stop_at || 'N/A'}*`
        );
