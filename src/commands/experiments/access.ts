...
const grantUserCommand = new Command('grant-user')
  ...
  .action(
    withErrorHandling(async (id: ExperimentId, options: { user: UserId; role: AssetRoleId }) => {
      const globalOptions = getGlobalOptions(grantUserCommand);
      const client = await getAPIClientFromOptions(globalOptions);
      await grantExperimentAccessUser(client, {
        experimentId: id,
        userId: options.user,
        roleId: options.role,
      });
...
const revokeUserCommand = new Command('revoke-user')
  ...
  .action(
    withErrorHandling(async (id: ExperimentId, options: { user: UserId; role: AssetRoleId }) => {
      ...
      const client = await getAPIClientFromOptions(globalOptions);
      await revokeExperimentAccessUser(client, {
        experimentId: id,
        userId: options.user,
        roleId: options.role,
      });
...
const grantTeamCommand = new Command('grant-team')
  ...
  .requiredOption('--role <assetRoleId>', 'asset role ID', parseAssetRoleId)
  .action(
    withErrorHandling(async (id: ExperimentId, options: { team: TeamId; role: AssetRoleId }) => {
      const globalOptions = getGlobalOptions(grantTeamCommand);
      const client = await getAPIClientFromOptions(globalOptions);
      await grantExperimentAccessTeam(client, {
        experimentId: id,
        teamId: options.team,
        roleId: options.role,
      });
...
const revokeTeamCommand = new Command('revoke-team')
  ...
  .action(
    withErrorHandling(async (id: ExperimentId, options: { team: TeamId; role: AssetRoleId }) => {
      ...
      const client = await getAPIClientFromOptions(globalOptions);
      await revokeExperimentAccessTeam(client, {
        experimentId: id,
        teamId: options.team,
        roleId: options.role,
      });
