...
const grantUserCommand = new Command('grant-user')
  ...
  .action(
    withErrorHandling(async (id: GoalId, options: { user: UserId; role: AssetRoleId }) => {
      ...
      printResult(globalOptions, {
        message: `✓ User ${options.user} granted access to goal ${id}`,
        id,
      });
    })
  );
...
const revokeUserCommand = new Command('revoke-user')
  ...
  .action(
    withErrorHandling(async (id: GoalId, options: { user: UserId; role: AssetRoleId }) => {
      ...
      printResult(globalOptions, {
        message: `✓ User ${options.user} access revoked from goal ${id}`,
        id,
      });
    })
  );
...
const grantTeamCommand = new Command('grant-team')
  ...
  .requiredOption('--role <assetRoleId>', 'asset role ID', parseAssetRoleId)
  .action(
    withErrorHandling(async (id: GoalId, options: { team: TeamId; role: AssetRoleId }) => {
      ...
      printResult(globalOptions, {
        message: `✓ Team ${options.team} granted access to goal ${id}`,
        id,
      });
    })
  );
...
const revokeTeamCommand = new Command('revoke-team')
  ...
  .action(
    withErrorHandling(async (id: GoalId, options: { team: TeamId; role: AssetRoleId }) => {
      ...
      printResult(globalOptions, {
        message: `✓ Team ${options.team} access revoked from goal ${id}`,
        id,
      });
    })
  );
