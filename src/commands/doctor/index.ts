      ...
      try {
        ...
        try {
          ...
          if (mode === 0o600) {
            ...
            console.log(
              chalk.yellow('⚠') + ` Credentials file permissions are ${modeStr} (expected 600)`
            );
            hasWarnings = true;
          }
        } catch (e) {
          if (e instanceof Error && (e as NodeJS.ErrnoException).code !== 'ENOENT') {
            console.log(
              chalk.yellow('⚠') + ` Could not check credentials file permissions: ${e.message}`
            );
            hasWarnings = true;
          }
        }
        ...
        if (apiKey) {
          try {
            ...
            if (staleCount > 0) {
              console.log(
                chalk.yellow('⚠') + ` ${staleCount} experiments stuck in 'created' state`
              );
              hasWarnings = true;
            } else {
            ...
            console.log(
              chalk.yellow('⚠') +
                ` Could not check for stale experiments: ${e instanceof Error ? e.message : e}`
            );
            hasWarnings = true;
          }
        }
