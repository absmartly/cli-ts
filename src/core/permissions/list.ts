...
export async function listPermissions(client: APIClient): Promise<CommandResult<unknown>> {
  const data = await client.listPermissions();
  return { data };
}

export async function listPermissionCategories(client: APIClient): Promise<CommandResult<unknown>> {
