...
const related = [
  {
    id: 2, name: 'r2', state: 'stopped',
    started_at: '2024-01-01', stopped_at: '2024-02-01',
    primary_metric_id: 100, primary_metric: { name: 'Conversions' },
  },
  {
    id: 3, name: 'r3', state: 'running',
    started_at: '2024-03-01', stopped_at: null,
    primary_metric_id: 100, primary_metric: { name: 'Conversions' },
  },
  {
    id: 4, name: 'r4', state: 'running',
    started_at: '2024-04-01', stopped_at: null,
    primary_metric_id: 999, primary_metric: { name: 'Other' }, // different metric — no fetch
  },
];
...
describe('summarizeRelatedExperiments', () => {
  ...
  it('caps at 24 items', async () => {
    const many = Array.from({ length: 30 }, (_, i) => ({
      id: i + 100, name: `e${i}`, state: 'running', primary_metric_id: 999,
    }));
    const client = { getExperimentMetricsCached: vi.fn() };
    const reg = new SourceSignalRegistry();
  ...
  it('records failures via registry and emits null impact for that related experiment', async () => {
    ...
    expect(reg.toArray().some(e => e.covers === 'related_experiments[3]' && /error/.test(e.source))).toBe(true);
  });
