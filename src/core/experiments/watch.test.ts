...
describe('watchExperimentTick', () => {
  ...
  it('returns hasMetrics: true with formatted results when metrics exist', async () => {
    const metricInfos = [{ id: 10, name: 'Conversion Rate', type: 'primary', effect: 'positive' }];
    ...
    const mockResults = [
      {
        ...
        variants: [
          ...
          },
          {
            ...
            variance: 0.02,
            abs_impact: 0.05,
            abs_impact_lower: 0.01,
            abs_impact_upper: 0.09,
          },
        ],
      },
    ];
    const mockFormattedRow = {
    ...
    expect(metricResults.fetchAllMetricResults).toHaveBeenCalledWith(
      mockClient,
      id(1),
      metricInfos
    );
